...
import com.example.fypapp.CustomActivity;
...
public class Feedback extends AppCompatActivity {
                        ...
                        try {
                            ...
                            runOnUiThread(new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    storeFeedbackInDatabase(userFeedback, sentimentScore, positivePercentage, isPositiveFeedback);
                                    ...
                                    updateActivityPopularity(activityName, isPositiveFeedback);

                                    saveFeedbackAndFinish(Activities.class.getSimpleName(), isPositiveFeedback, positivePercentage);
                                    saveFeedbackAndFinish(DefaultActivities.class.getSimpleName(), isPositiveFeedback, positivePercentage);
                                    saveFeedbackAndFinish(PositiveActivities.class.getSimpleName(), isPositiveFeedback, positivePercentage);
                                }
                            });
    ...
    private void storeFeedbackInDatabase(String feedback, float sentimentScore, float positivePercentage, boolean isPositiveFeedback) {
    ...
    private void updateActivityPopularity(String activityName, boolean isPositiveFeedback) {
        FirebaseUser user = FirebaseAuth.getInstance().getCurrentUser();
        if (user != null) {
            String userId = user.getUid();
            DatabaseReference activitiesRef = FirebaseDatabase.getInstance().getReference("user_activities").child(userId);

            Query query = activitiesRef.orderByChild("name").equalTo(activityName);
                ...
                public void onDataChange(@NonNull DataSnapshot snapshot) {
                    if (snapshot.exists()) {
                        for (DataSnapshot activitySnapshot : snapshot.getChildren()) {
                            CustomActivity activity = activitySnapshot.getValue(CustomActivity.class);

                            if (activity != null) {
                                activity.setPopular(isPositiveFeedback);
                                activitySnapshot.getRef().setValue(activity);
                            }
                        }
                    }
                }
    ...
    private void saveFeedbackAndFinish(String activityName, boolean isPositiveFeedback, float positivePercentage) {
        Intent resultIntent = new Intent();
        resultIntent.putExtra("activityName", activityName);
        resultIntent.putExtra("isPositiveFeedback", isPositiveFeedback);
        resultIntent.putExtra("positivePercentage", positivePercentage);
        setResult(RESULT_OK, resultIntent);

        runOnUiThread(() -> {
            txvResult.setText("Feedback submitted successfully for " + activityName);
        });
